...
generate_vac_schedule <- function( VAX.INITIAL.STORAGE.NUM,
    ...
    df <- data.frame(V,OPT.VAX.RATE,V2)
    colnames(df) <- c("Stock","Vax1","Vax2")
    df$t <- as.numeric(row.names(df))
    return(df)
}

plot_vac_schedule.2 <- function (df){
    sc <- max(df$Stock)/max(df$Vax1)
...
#                             MAX.VAC.RATE,VAX.WINDOW.DAYS,SECOND.VAX.LOSS.FRAC,MAX.TIME.DAYS)
