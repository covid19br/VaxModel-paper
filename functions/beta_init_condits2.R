...
init_condits <- function(r, NEW.HOSP.AGE, PREVALENCE, POP.DISTR, CONTACT.M, EXPOSURE.PERIOD.DAYS,
  ...
  POP.E1 <- EXPOSURE.PERIOD.WEEKS * NEW.HOSP.AGE / SEVERITY.FRAC

  POP.INF1 <- POP.E1 * POP.AUTO
  # Colocar sum gera problemas de população negativa em idosos
  POP.S <- POP.DISTR - (POP.INF1[1:3] + POP.INF1[4:6] + POP.INF1[7:9] + POP.INF1[10:12] + POP.D1 + POP.R1)

  POP0 <- c(POP.S, POP.INF1, POP.R1, POP.D1)
