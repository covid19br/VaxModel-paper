...
# source('./functions/dados_estados.R')
source('./functions/beta_init_condits2.R')
source('./functions/fitP.exp.R')
...
POP.ESTADO.REL.FRAC <- all.age.distr[1, Estado] / sum(all.age.distr[1,-1])
...
BETA.RATE              = 1    # Average contact rate and success between unvaccinated susceptibles and infectious individuals \beta
...
### TO-DO: valores estão estranhos, ainda a revisar
...
# state.zoo.list <- carregar_dados_estado(estado = Estado)
load("DATA/dados_casos_SP.RData")
...
BETA.RATE <- init.conds$BETA.RATE
POP0 <- c(POP0,rep(0,2*length(POP0)))
names(POP0) <- paste0(rep(c("POP.S", "POP.E", "POP.A", "POP.I", "POP.H",
                             "POP.R", "POP.D","POP.Sv", "POP.Ev", "POP.Av", "POP.Iv", "POP.Hv",
                            "POP.Rv", "POP.Dv","POP.Sw", "POP.Ew", "POP.Aw", "POP.Iw", "POP.Hw",
                            "POP.Rw", "POP.Dw"), each=3), rep(1:3, 21))
